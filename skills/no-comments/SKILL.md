...
Spawn the comment reviewer. Act on accepted findings.

Authoring agents defend comments. Defer to the comment reviewer's fresh perspective.
...
1. Read [`references/comment-reviewer.md`](references/comment-reviewer.md). When delegation is available, give that lens and the scope to an independent read-only reviewer; otherwise apply the lens inline.
