...
Launch all reviewers in a single message using the host delegation capability. Use the `interrogate reviewers` list from `.pstack/models.md` when present, one reviewer per entry, extending or shrinking the Reviewer A/B/C/D labels below to the configured entry count; otherwise use the table defaults.

| Subagent | Default model |
|----------|---------------|
| Reviewer A | `judgment-model` |
| Reviewer B | `deep-code-model` |
| Reviewer C | `fast-code-model` |
| Reviewer D | `independent-review-model` |
