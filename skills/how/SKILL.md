...
- Start broad: enumerate relevant directories and search for key types, interfaces, and class names
...
The worker does its own exploration (file discovery, text search, and focused reads) and writes the explanation directly. Read `references/explainer-prompt.md` for the communication style and output format. Same structure, just no explorer findings as input.

Proceed to Step 4.

### Step 3. Synthesize (complex questions only)

Once all explorers return, spawn a single delegated worker to synthesize their findings into one coherent explanation:
...
After the explanation is complete, spawn one architectural critic per model in your configured how-critics list (defaults `judgment-model`, `deep-code-model`, `fast-code-model`, `independent-review-model`), all in a single message.
