...
Maintain a plan or checklist with one entry per phase before starting. Autonomous mode without checkpoints needs the list to show phase position and keep phases from silently disappearing.
...
Use your configured architect runners (defaults `judgment-model`, `deep-code-model`, `fast-code-model`, `independent-review-model`).
