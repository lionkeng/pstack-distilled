...
Open a plan or checklist with one item per step below.
...
State your read of scope and constraints in one paragraph. Use the host's structured user-interaction capability only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); give concrete options with each open question.
...
- Instruct delegated implementers to read and apply poteto-mode first. If the host cannot preload a worker profile, include that requirement in the brief.
- Pass `model:` explicitly per the configured roles (defaults `fast-code-model` for code, `judgment-model` for judgment).
...
If a phase creates or edits a skill, the phase instructs the implementer to use an Agent Skills-compatible authoring and validation workflow.
...
**Runtime.** Exercise the feature on the matching surface with the project's available verification harness:
...
- No verification harness for the touched surface: flag it in the plan.
...
- prose-cleanup workflow over each diff before commit. the **unslop** skill over any prose surface.
