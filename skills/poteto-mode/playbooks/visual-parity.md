...
4. Verify each component against its baseline via image diff on the matching surface with the project's available verification harness. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. `recurring-run capability` per component until the diff is zero.
