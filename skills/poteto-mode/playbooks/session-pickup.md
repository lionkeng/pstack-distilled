...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <conversation record path>", "you're taking over", "pick up where X left off", a remote-worker URL handoff, or a pushed branch you're meant to continue.
...
1. Locate the prior trail. A conversation record explicitly exposed by the host for the active workspace, a remote-worker URL, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long conversation record in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
