...
6. **Verify the chain from conversation records, not self-report.** Use only each candidate's conversation record when the host explicitly exposes it for the active workspace. Never probe private host storage or another workspace. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
